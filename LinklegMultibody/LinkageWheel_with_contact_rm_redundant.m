...
history.MatrixR = zeros(6*n, dof , time.step+1);
...
% Projection Method
% MatrixB: constant
% MatrixR: we can get MatrixR by inv([Jb ; MatrixB]) and MatrixB is the
% last #dof columns of the inverse matrix.
MatrixR = zeros(6*n, dof);
MatrixB = zeros(dof, 6*n);
...
%     J_A1_a1        = [  eye(3)  -skew(vector.alpha1_A1)   ; zeros(1,3)   (skew(ns(:,1,2))*ns(:,2,4))']; % xy
%     J_A1_g1        = [ -eye(3)   skew(vector.gamma1_A1)   ; zeros(1,3)  -(skew(ns(:,1,2))*ns(:,2,4))']; % xy
...
%     J_A2_g2        = [ -eye(3)   skew(vector.gamma2_A2)   ; zeros(1,3)  -(skew(ns(:,1,3))*ns(:,2,5))']; % xy
    ...
    % E gamma1(4)/gamma2(5)-2 constraints
    % 2 translation
...
%     J_E_g2         = [ -eye(3)   skew(vector.gamma2_E)    ; zeros(1,3)  -(skew(ns(:,1,4))*ns(:,2,5))']; % xy
    ...
    J_E_g1         = [ Exz  [-skew_vector.gamma1_E(1,:) ; -skew_vector.gamma1_E(3,:)] ];
    J_E_g2         = [-Exz  [ skew_vector.gamma2_E(1,:) ;  skew_vector.gamma2_E(3,:)] ];
...
%     J_C1_d1        = [ -eye(3)   skew(vector.delta1_C1)   ; zeros(1,3)  -(skew(ns(:,1,6))*ns(:,2,8))']; % xy
    ...
    J_C1_b1        = [  Exz  [-skew_vector.beta1_C1(1,:)  ; -skew_vector.beta1_C1(3,:)]  ];
    J_C1_d1        = [ -Exz  [ skew_vector.delta1_C1(1,:) ;  skew_vector.delta1_C1(3,:)] ];    
...
%     J_C2_d2        = [ -eye(3)   skew(vector.delta2_C2)   ; zeros(1,3)  -(skew(ns(:,1,7))*ns(:,2,9))']; % xy
    ...
    J_C2_b2        = [  Exz  [-skew_vector.beta2_C2(1,:)  ; -skew_vector.beta2_C2(3,:) ]  ];
    J_C2_d2        = [ -Exz  [ skew_vector.delta2_C2(1,:) ;  skew_vector.delta2_C2(3,:)]  ];    
...
%     J_G_e2         = [ -eye(3)   skew(vector.epsilon2_G)  ; zeros(1,3)  -(skew(ns(:,1,10))*ns(:,2,11))']; % xy
    ...
    J_G_e1         = [  Exz  [-skew_vector.epsilon1_G(1,:) ; -skew_vector.epsilon1_G(3,:)]];
    J_G_e2         = [ -Exz  [ skew_vector.epsilon2_G(1,:) ;  skew_vector.epsilon2_G(3,:)]];
...
%     Js_BASE_trans = [[1;0;0]'   1/2*(skew(-s(1:3,1))*[1;0;0])' ; [0;1;0]'   1/2*(skew(-s(1:3,1))*[0;1;0])']; % Pickl_MT_2009
    ...
    % Jacobian of Body, alpha1,2, gamma1,2, beta1,2, delta1,2, epsilon1,2 - remove redundancy
...
%          Jr_O2_B  Jr_O2_a2         x        x        x         x        x         x        x         x          x;
    ...
    % Jacobian of Body, alpha1,2, gamma1,2, beta1,2, delta1,2, epsilon1,2
%     Jb = [J_O1_B   J_O1_a1         X        X        X         X        X         X        X         X          X;
%           J_O2_B         X   J_O2_a2        X        X         X        X         X        X         X          X;
%                X   J_A1_a1         X  J_A1_g1        X         X        X         X        X         X          X;
%                X         X   J_A2_a2        X  J_A2_g2         X        X         X        X         X          X;
...
%          ];       
    ...
    w0b = zeros(5*nc + no + n_fix - nr, 1);
    
    rank(Jb);

    % acceleration analysis
    % B1 alpha1(2)/beta1(6)
    temp_rot1 = (skew(skew(v(10:12,1))*ns(:,1,2))*ns(:,2,6))'*v(10:12,1);
    temp_rot2 = (skew(ns(:,1,2))*(skew(v(34:36,1))*ns(:,2,6)))'*v(10:12,1);
    temp_rot3 = (skew(skew(v(10:12,1))*ns(:,1,2))*ns(:,2,6))'*v(34:36,1);
    temp_rot4 = (skew(ns(:,1,2))*(skew(v(34:36,1))*ns(:,2,6)))'*v(34:36,1);

    temp_rot5 =  ( (ns(:,2,6)'*v(10:12,1) )*ns(:,1,2)-( ns(:,1,2)'*v(34:36,1))*ns(:,2,6) )'*v(10:12,1);
    temp_rot6 = -( (ns(:,2,6)'*v(10:12,1) )*ns(:,1,2)-( ns(:,1,2)'*v(34:36,1))*ns(:,2,6) )'*v(34:36,1);

    temp_rot = temp_rot1 + temp_rot2 - temp_rot3 - temp_rot4;
    %% Matrix B
    % Matrix R: project dependent velocities to independent velocities
    % Matrix B is an orthogonal matrix
    % Matrix R = inv([Jb ; MatrixB]);
    if t == 1
        [qrd.Q,qrd.R] = qr(Jb');
        MatrixB = qrd.Q(:,end-dof+1 : end)';
    end
    
    if (rank([Jb ; MatrixB]) ~= size(Jb,2))
        disp('Matrix B needs to be recalculated !');
        [qrd.Q,qrd.R] = qr(Jb');
        MatrixB = qrd.Q(:,end-dof+1 : end)';
    else
        disp('Matrix B remain the same');
    end

    matrixSR_ = inv([Jb ; MatrixB]); % [S R]
    MatrixR = matrixSR_(:,end - dof + 1 : end);
...
%     % Jacobian
%     Jn = [z    z    z    z    z    z    z    z    z       Jc_G_e1             z;
%           z    z    z    z    z    z    z    z    z             z       Jc_G_e2];
% 
%     % wn
%     wn = Jn*v;
%     w0n = [0;0];
