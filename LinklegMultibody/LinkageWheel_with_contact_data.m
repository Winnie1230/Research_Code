...
%%
VICON_FILEPATH = 'C:\Users\BioRoLaT470S\Documents\r09522826\Experiment\Linkleg_48V\0331\20220331_Vicon實驗\';
MOTOR_FILEPATH = 'C:\Users\BioRoLaT470S\Documents\r09522826\Experiment\Linkleg_48V\0331\';
FILENAME = '20220331_SineWave_0.5Hz_test1';

% load
aligned_data = readmatrix([MOTOR_FILEPATH,'DataAligned\',FILENAME,'_aligned.csv']);
% load("LinkPara_R100.mat");

%% 
% sinewave frequency
freq = str2double(FILENAME(19:22));
index.Vicon_trigger = find(aligned_data(:,11))+(1/freq)*2;
% index.Motor_startVJ = find(alignedshift_data(:,10) == 1);
% index.start_plot = index.Motor_startVJ - 10;
% index.Vicon_LO = find(alignedshift_data(:,10) == 2);
% index.end_plot = index.Vicon_LO + 5;
...
time.period = 0.002;
% time.step = 1;
% time.step = index.Vicon_LO-index.Motor_startVJ+1;
% time.step = index.end_plot-index.Motor_startVJ+1;
time.step = round(1/0.75/time.period)+50;
...
FILEPATH = 'C:\Users\BioRoLaT470S\Documents\r09522826\Simulation\Matlab_MBD\0221\';
FILENAME = 'MBD_constTorq_2.5N';
SAVEFILE = 1; %1: save to csv(remember to change), 0: don't save
...
beta1.I = [   91161.661859   -774.501609      37316.909503;
             -774.501609      517848.015598  -194.724041;
              37316.909503   -194.724041      494363.523385];
...
%              772.772852    517848.015598	 -201.475342  ;
%         	-40829.647339	 -201.475342	 493681.548715];
beta2.I = [   91161.661859   774.501609     -37316.909503;
              774.501609     517848.015598  -194.724041;
             -37316.909503   -194.724041    494363.523385];
...
delta1.m = 5.580595;
delta1.I =  [   125.289199  0.000000      0.000000;
                0.000000    4725.245412   0.000000;
                0.000000    0.000000      4692.966127];
...
delta2.m = 5.580595;
delta2.I =  [   125.289199  0.000000      0.000000;
                0.000000    4725.245412   0.000000;
                0.000000    0.000000      4692.966127];
...
% ---------- pin G(Rigid Body) ----------
pinG.m = 1; 
pinG.h = 100; % Height (Assume B is a solid cuboid)
pinG.w = 100; % Width
pinG.d = 100; % Depth
pinG.I = pinG.m/12 * [ pinG.h^2+pinG.d^2 0 0 ; 0 pinG.w^2+pinG.d^2 0 ; 0 0 pinG.w^2+pinG.h^2 ]; % Inertia Tensor
pinG.W = [(1/BASE.m)*eye(3) zeros(3,3);zeros(3,3) inv(pinG.I)]; % Inverse inertia matrix
pinG.theta = 0;
pinG.q = [cos(pinG.theta/2) 0 0 sin(pinG.theta/2)]'; % Orientation of mass B (quaternion)
pinG.x = points.G_3d;
% ---------------------------------------
...
%     wrench.ext(16:18,1) = Torq_a2;
    wrench.ext(11,1) = km*aligned_data(t+index.Vicon_trigger-1,4)*1e+9;
    wrench.ext(17,1) = -km*aligned_data(t+index.Vicon_trigger-1,4)*1e+9;
    ...
    %% BASE CONSTRAINTS
    % fix BASE(B) to O
    J_BASE_O = [eye(3) -skew(BASE.B_O) zeros(3,(n-1)*6) ; zeros(3,3) eye(3) zeros(3,(n-1)*6)];
...
%          Jr_O2_B  Jr_O2_a2         x        x        x         x        x         x        x         x          x;
    ...
    % O1 B(1)/alpha1(2)
    Phi.O1_trans = s(8:10,1) + vector.alpha1_O1 - s(1:3,1) - BASE.B_O; % translation
    Phi.O1_rot = [ns(:,1,1)'*ns(:,2,2) ; ns(:,3,1)'*ns(:,2,2)]; % rotation

    % O2 B(1)/alpha2(3)
    Phi.O2_trans = s(15:17,1) + vector.alpha2_O2 - s(1:3,1)  - BASE.B_O;
...
% if(SAVEFILE)
%     writematrix([((0:1:time.step)*time.period)',history.Fn,history.theta],[FILEPATH,FILENAME,'.csv']);
% end


if t < time.step
    history.Fn(t+1:end,1) = 0;
    history.theta(t+1:end,1) = 160;
    history.O(t+1:end,:) = nan;
end

%% virtual work
% reply.rmotor = alignedshift_data(index.Motor_startVJ-1:index.Vicon_LO,3)*180/pi;
% reply.lmotor = (alignedshift_data(index.Motor_startVJ-1:index.Vicon_LO,6)-2*pi)*180/pi;
reply.rmotor = aligned_data(index.Motor_startVJ-1:index.end_plot,3)*180/pi;
reply.lmotor = (aligned_data(index.Motor_startVJ-1:index.end_plot,6)-2*pi)*180/pi;

theta0_deg = 17;
phi_r = reply.rmotor + theta0_deg;
phi_l = reply.lmotor - theta0_deg;

theta = 1/2*(phi_r-phi_l);
beta = 1/2*(phi_r+phi_l);
Fn_vw = zeros(time.step,1);
dtheta = 0.001;

for u = 1 : time.step+1
    mtorq_direct = km*aligned_data(u+index.Motor_startVJ-2,4);
    mtorq_belt = -km*aligned_data(u+index.Motor_startVJ-2,7);
    theta_vw = theta(u,1);
    beta_vw = 0;
     
    points1 = calculate_coordinate_flat2(theta_vw,beta_vw);
    points2 = calculate_coordinate_flat2(theta_vw+dtheta,beta_vw);
    dGz = (points1.G(2) - points2.G(2))*1e-3;

    Fn_vw(u,1) = -((mtorq_direct + mtorq_belt)*dtheta / dGz);
end

Fn_vw(end-(index.end_plot-index.Vicon_LO+1):end,1) = 0;
...
time.T = (0:1:t)'*time.period;

% h(1) = subplot(6,1,1);
subaxis(6,1,1,'MarginTop',0.025,'MarginRight',0.05);
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,2),'LineWidth',1.5,'DisplayName','Position command(R)','Color','b'); hold on
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,3),'LineWidth',1.5,'DisplayName','Encoder(R)','LineStyle','--','Color','#D95319');
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,5),'LineWidth',1.5,'DisplayName','Position command(L)','Color','#7E2F8E');
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,6),'LineWidth',1.5,'DisplayName','Encoder(L)','LineStyle','-.','Color','#77AC30');
...
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,4),'Color','#77AC30','LineWidth',1.5,'DisplayName','Exp-current R'); hold on
plot(time.T,aligned_data(index.Vicon_trigger:index.Vicon_trigger+t,7),'LineStyle','-.','Color','#D95319','LineWidth',1.5,'DisplayName','Exp-current L');
xline((index.Vicon_LO-index.Motor_startVJ)*time.period,'Color','#EDB120','LineWidth',1.5,'LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','HandleVisibility','off','Label','Exp lift-off');
hold off
...
temp = aligned_data(:,11)-mean(aligned_data(1:300,9));
...
plot(time.T,aligned_data(index.Motor_startVJ-1:index.end_plot,8),'Color','#CE0000','LineStyle','-','LineWidth',1.5,'DisplayName','GRF-Exp');
...
gcf_arr = [0.3,0.1,0.4,0.25];
set(gcf,'unit','normalized','position',gcf_arr);

% subplot(2,1,1);
plot(time.T,history.PFext*1e-9,'LineWidth',1.5,'LineStyle','-','DisplayName','Power of Fext'); hold on
plot(time.T,history.Pinertial*1e-9,'LineWidth',1.5,'LineStyle','--','DisplayName','Power of inertial force');
plot(time.T,history.PFn*1e-9,'LineWidth',1.5,'LineStyle','-.','DisplayName','Power of Fn');
plot(time.T,history.Pg*1e-9,'LineWidth',1.5,'LineStyle',':','DisplayName','Power of Fg');
xlim([0 time.step*time.period]);
hold off
title('Power-t','Interpreter','latex');
xlabel('t[sec]','Interpreter','latex');
ylabel('Power[W]','Interpreter','latex');
legend('Location','northeast','Interpreter','latex','Orientation','vertical');

% subplot(2,1,2);
% plot(time.T,history.Pinertial*1e-9,'LineWidth',1.5,'LineStyle','-');hold on
% plot(time.T,history.Pinertial_BASE*1e-9,'LineWidth',1.5,'LineStyle','--','Color','#77AC30'); 
% plot(time.T,(history.Pinertial-history.Pinertial_BASE)*1e-9,'LineWidth',1.5,'LineStyle','-.');
% xlim([0 time.step*time.period]);
% hold off