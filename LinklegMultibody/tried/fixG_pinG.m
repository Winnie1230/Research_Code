...
ng_fix = 6; % fix pointG to the ground(= regard pointG as Hinge Joint or Ball-And-Socket Joint?)
...
pinG.W = [(1/B.m)*eye(3) zeros(3,3);zeros(3,3) inv(pinG.I)]; % Inverse inertia matrix
pinG.theta = 0;
...
for t = 1:time.step
    ...
    % ball-and-socket joint end of alpha1,2
    Jbs_B1_a1 = [  eye(3)  -skew(vector.alpha1_B1)];
    Jbs_B2_a2 = [  eye(3)  -skew(vector.alpha2_B2)];
    ...
    % ball-and-socket joint end of E
    Jbs_E_g1 = [  eye(3)  -skew(vector.gamma1_E)];
    Jbs_E_g2 = [  eye(3)  -skew(vector.gamma2_E)];
    ...
    % ball-and-socket joint of beta1,2
    Jbs_C1_b1 = [  eye(3)  -skew(vector.beta1_C1)];
    Jbs_C2_b2 = [  eye(3)  -skew(vector.beta2_C2)];
    ...
    % ball-and-socket joint of epsilon1,2
    Jbs_G1_e1   = [  eye(3)  -skew(vector.epsilon1_G)];
    Jbs_G1_pinG = [ -eye(3)   zeros(3,3)];
    Jbs_G2_e2   = [  eye(3)  -skew(vector.epsilon2_G)];
    Jbs_G2_pinG = [ -eye(3)   zeros(3,3)];

    % contact joint of epsilon1,2
%     Jcj_G1_e1   = [ [0;0;1]'  (skew(vector.epsilon1_G)*[0;0;1])'];
%     Jcj_G1_pinG = [-[0;0;1]'  -zeros(1,3)                       ];
%     Jcj_G2_e2   = [ [0;0;1]'  (skew(vector.epsilon2_G)*[0;0;1])'];
%     Jcj_G2_pinG = [-[0;0;1]'  -zeros(1,3)                       ];
...
%     Js_BASE_trans = [ns(:,1,1)'   (skew(s(1:3,1))*ns(:,1,1)+2*skew(B.anchor-s(1:3,1))*ns(:,1,1))' ; ns(:,2,1)'   (skew(s(1:3))*ns(:,2,1)+2*skew(B.anchor-s(1:3,1))*ns(:,2,1))'];
    ...
    % fix BASE(add a ball-and-socket joint)
    Jbs_BASE = [  eye(3)  zeros(3)];

    %% End effect CONSTRAINTS
...
%         Js_BASE         X         X        X        X         X        X         X        X         X          X             X; 
              z         z         z        z        z         z        z         z        z  Jbs_G1_e1         z  Jbs_G1_pinG;
              z         z         z        z        z         z        z         z        z         z  Jbs_G2_e2  Jbs_G2_pinG];       
...
%               z         z         z        z        z         z        z         z        z    Jpc_e1        z;
%               z         z         z        z        z         z        z         z        z         z   Jpc_e2];  
...
%               X         X         X  J_D1_g1        X          X          X   J_D1_d1        X;
%               X         X         X        X  J_D2_g2          X          X         X  J_D2_d2;
%               z         z         z        z        z  Jbs_C1_b1          z         z        z;
%               z         z         z        z        z          z  Jbs_C2_b2         z        z];  
...
%     % Jacobian of Body, alpha1,2(freely) slider joint
%     J = [ 
% %         Js_BASE          X            X;
% %          Jbs_BASE        z          z;
%            J_O1_B    J_O1_a1          X;
...
%            J_O2_B        X  J_O2_a2];
    ...
    % Jacobian of Body (slider joint + ball-and-socket joint)
%     J = [Js_BASE ; Jbs_BASE];

    % Jacobian of Body (slider joint)
%     J = Js_BASE;

    % Jacobian of Body (with no constraints)
%     J = [];
...
%     bias = beta / time.period * C;
    ...
    %     lambda(:,1) = -K\(J*v_+bias) / time.period;
        ...
        wrench.total = wrench.ext + wrench.g;
    end
...
%     F_BASE = J_BASE_O'*lambda(1:6,1);
%     Fbs_BASE = Jbs_BASE'*lambda(6:8,1);
...
%     Fbs_B1_a1 = Jbs_B1_a1'*lambda(11:13,1);
%     Fbs_B2_a2 = Jbs_B2_a2'*lambda(14:16,1);

%     Fbs_E_g1 = Jbs_E_g2'*lambda(26:28,1);
%     Fbs_E_g2 = Jbs_E_g2'*lambda(29:31,1);
%                                         
%     Fbs_E_g1+Fbs_E_g2;

%     Fbs_C1_b1 = Jbs_C1_b1'*lambda(56:58,1);
%     Fbs_C2_b2 = Jbs_C2_b2'*lambda(59:61,1);
%     Fbs_C1_b1+Fbs_C2_b2;

    Fbs_G1_e1 = Jbs_G1_e1'*lambda(end-5:end-3,1);
    Fbs_G1_pinG = Jbs_G1_pinG'*lambda(end-5:end-3,1);
    Fbs_G2_e2 = Jbs_G2_e2'*lambda(end-2:end,1);
    Fbs_G2_pinG = Jbs_G2_pinG'*lambda(end-2:end,1);
    Fbs_sum = Fbs_G1_e1+Fbs_G2_e2;

    wrench.sum_force = zeros(6,1);
    for i = 0 : n-1
        wrench.sum_force = wrench.sum_force + wrench.total(i*6+1 : i*6+6,1);
    end
    ...
    %% Plot
%     PlotCuboid1(s(1:3,1),B.w,B.d,B.h,Rq(s(4:7,1))  ,'#EAC100','#977C00');
...
%     arrow3d([s(71,1) s(71,1)+vector.epsilon2_G(1)],[s(72,1) s(72,1)+vector.epsilon2_G(2)],[s(73,1) s(73,1)+vector.epsilon2_G(3)],a,b,c,'r');
   ...
   % plot forces
    scale = 1e-5;
    arrow3d([s(64,1)+vector.epsilon1_G(1) s(64,1)+vector.epsilon1_G(1)+scale*Fbs_G1_e1(1)],[s(65,1)+vector.epsilon1_G(2) s(65,1)+vector.epsilon1_G(2)+scale*Fbs_G1_e1(2)],[s(66,1)+vector.epsilon1_G(3) s(65,1)+vector.epsilon1_G(3)+scale*Fbs_G1_e1(3)],a,b,c,'k');
    arrow3d([s(71,1)+vector.epsilon2_G(1) s(71,1)+vector.epsilon2_G(1)+scale*Fbs_G2_e2(1)],[s(72,1)+vector.epsilon2_G(2) s(72,1)+vector.epsilon2_G(2)+scale*Fbs_G2_e2(2)],[s(73,1)+vector.epsilon2_G(3) s(73,1)+vector.epsilon2_G(3)+scale*Fbs_G2_e2(3)],a,b,c,'k');
%     arrow3d([s(78,1) s(78,1)+scale*Fcj_G1_e1(1)],[s(79,1) s(79,1)+scale*Fcj_G1_e1(2)],[s(80,1) s(80,1)+scale*Fcj_G1_e1(3)],a,b,c,'#5B00AE');
%     arrow3d([s(78,1) s(78,1)+scale*Fcj_G2_e2(1)],[s(79,1) s(79,1)+scale*Fcj_G2_e2(2)],[s(80,1) s(80,1)+scale*Fcj_G2_e2(3)],a,b,c,'#007500');
