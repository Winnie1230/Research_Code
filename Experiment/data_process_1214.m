...
csv = load('C:\Users\BioRoLaT470S\Documents\r09522826\Experiment\Linkleg_48V\1214\1214_violentjump11.csv');
...
threshold = 0.05;
mod_poscmd1 = data(:,3);
mod_poscmd1(mod_poscmd1 <= threshold) = 2*pi-mod_poscmd1(mod_poscmd1 <= threshold);
mod_poscmd2 = data(:,8);
mod_poscmd2(mod_poscmd2 <= threshold) = 2*pi-mod_poscmd2(mod_poscmd2 <= threshold);
mod_pos1 = data(:,5);
mod_pos1(mod_pos1 <= threshold) = 2*pi-mod_pos1(mod_pos1 <= threshold);
mod_pos2 = data(:,10);
mod_pos2(mod_pos2 <= threshold) = 2*pi-mod_pos2(mod_pos2 <= threshold);

subplot(3,1,1);
% plot(data(:,1),data(:,3),'Color','#0072BD','DisplayName','position cmd','LineWidth',2); hold on
plot(data(:,1),mod_poscmd1,'Color','#0072BD','DisplayName','position cmd','LineWidth',2); hold on
...
% title({['24V slow motion no contact'],['Forward speed: 100 deg/s, Recover speed: 100 deg/s'],['Motor1(R) position']},'Interpreter','latex');
title({['48V slow motion no contact'],['Forward speed: 100 deg/s, Recover speed: 100 deg/s'],['Motor1(R) position']},'Interpreter','latex');
...
plot(data(:,1),mod_poscmd2,'Color','#7E2F8E','DisplayName','position cmd','LineWidth',2); hold on
